...
- **D-10: Monorepo layout.** Apply industry-standard layout: `python/` (uv workspace member with `pyproject.toml`), `typescript/` (npm workspace member with `package.json`), `directory/` (TypeScript Cloudflare Worker, npm workspace member), `spec/test-vectors/` (shared JSON test fixtures), `docs/` (Astro Starlight, deferred details to Phase 5), `.github/workflows/` (CI for python, typescript, cross-language conformance). Single repo, dual workspace roots (uv + npm). **Updated in Plan 01-02 (2026-05-03):** originally specified `pnpm` workspaces; switched to `npm` workspaces (root `package.json` with `workspaces` field, NOT `pnpm-workspace.yaml`) per user decision — pnpm not installed locally; minimising tooling surface for the 6+ months unmaintained period.
